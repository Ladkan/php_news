...
session_start();
    ...
    if(isset($_POST["email"], $_POST["password"])){
        ...
        if($user === false){
            ...
            die('chybne prihlasocavi udaje');
        }

        $passwd = password_verify($_POST['password'], $user['password']);
        if(!$passwd){
            header('Location: login');
            die('chynme prihlasovaci udaje');
        }
    ...
    <div class="container bg_white my-3">
        ...
        <header class="bg_space_cadet rounded-2">
            <nav class="menu p-2">
                ...
                <?php else: ?>
                <a href="login" class="nav-item active">Přihlásit se</a>
            ...
            <section class="row flex-row-reverse justify-content-center">
                <div class="col-4">
                    <form action="" method="post" class="row g-3">
                        ...
                        <button class="btn btn-oxford">Login</button>
                    </form>
                </div>
            </section>
        </main>
